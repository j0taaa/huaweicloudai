...
const groupOptionsByCategory = (options: OptionEntry[]) => {
  const grouped = new Map<string, OptionEntry[]>();
  for (const option of options) {
    if (SENSITIVE_OPTION_KEYS.has(option.key)) continue;
    const title = categoryTitle(option.key);
...
export default async function AdminPage({ searchParams }: AdminPageProps) {
  ...
  const optionGroups = groupOptionsByCategory(options);
  ...
  return (
    <div className="app-shell min-h-dvh w-full px-4 py-6 text-zinc-900 dark:text-zinc-50 sm:px-6 lg:px-8">
      <main className="mx-auto flex w-full max-w-5xl flex-col gap-5">
        ...
        {!authenticated ? (
          <form action={loginAction} className="surface-card flex max-w-xl flex-col gap-3 rounded-3xl border border-white/60 px-5 py-5 backdrop-blur sm:px-6">
            ...
            <button className="w-fit rounded-xl bg-zinc-900 px-4 py-2 text-sm font-semibold text-white dark:bg-zinc-100 dark:text-zinc-900" type="submit">Login</button>
          </form>
        ) : (
          <div className="grid gap-5 lg:grid-cols-2">
            <form action={saveCoreSettingsAction} className="surface-card flex flex-col gap-3 rounded-3xl border border-white/60 px-5 py-5 backdrop-blur sm:px-6">
              ...
              <button className="mt-2 w-fit rounded-xl bg-zinc-900 px-4 py-2 text-sm font-semibold text-white dark:bg-zinc-100 dark:text-zinc-900" type="submit">Save access settings</button>
            </form>

            <form action={saveCoreSettingsAction} className="surface-card flex flex-col gap-3 rounded-3xl border border-white/60 px-5 py-5 backdrop-blur sm:px-6">
              <h2 className="text-lg font-semibold">Inference Settings</h2>
              <label className="text-sm font-medium">Built-in inference URL</label>
              <input name="baseUrl" defaultValue={config.builtInInference.baseUrl} className="rounded-xl border border-zinc-200 bg-white/90 px-3 py-2 outline-none dark:border-white/15 dark:bg-black/30" />
              <label className="text-sm font-medium">Built-in inference API key</label>
              <input name="apiKey" defaultValue={config.builtInInference.apiKey} className="rounded-xl border border-zinc-200 bg-white/90 px-3 py-2 outline-none dark:border-white/15 dark:bg-black/30" />
              <label className="text-sm font-medium">Built-in inference model</label>
              <input name="model" defaultValue={config.builtInInference.model} className="rounded-xl border border-zinc-200 bg-white/90 px-3 py-2 outline-none dark:border-white/15 dark:bg-black/30" />
              <button className="mt-2 w-fit rounded-xl bg-zinc-900 px-4 py-2 text-sm font-semibold text-white dark:bg-zinc-100 dark:text-zinc-900" type="submit">Save inference settings</button>
            </form>

            <form action={changeAdminPasswordAction} className="surface-card flex flex-col gap-3 rounded-3xl border border-white/60 px-5 py-5 backdrop-blur sm:px-6">
              ...
              <button className="mt-2 w-fit rounded-xl bg-zinc-900 px-4 py-2 text-sm font-semibold text-white dark:bg-zinc-100 dark:text-zinc-900" type="submit">Update admin password</button>
            </form>
        ...
        {authenticated ? (
          <section className="surface-card rounded-3xl border border-white/60 px-5 py-5 backdrop-blur sm:px-6">
            ...
            {optionGroups.length === 0 ? <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-300">No options saved yet.</p> : (
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                {optionGroups.map(([group, groupedOptions]) => (
                  <div key={group} className="rounded-2xl border border-zinc-200/80 bg-white/70 p-3 dark:border-white/10 dark:bg-black/30">
                    <h3 className="text-sm font-semibold uppercase tracking-wide text-zinc-600 dark:text-zinc-300">{group}</h3>
                    <ul className="mt-2 space-y-2">
                      {groupedOptions.map((option) => (
                        <li key={option.key}>
                          <p className="font-mono text-xs font-semibold">{option.key}</p>
