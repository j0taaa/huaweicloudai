...
const renderAssistantMessageContent = (content: string): ReactNode[] => {
  ...
  while (match) {
    ...
    if (markdownContent) {
      blocks.push(
        <ReactMarkdown key={`markdown-${chartIndex}-${match.index}`} remarkPlugins={[remarkGfm]}>
          {markdownContent}
        </ReactMarkdown>,
      );
    }
    ...
    if (chartData) {
      blocks.push(<ChartBlock key={`chart-${chartIndex}-${match.index}`} data={chartData} />);
    } else {
      blocks.push(
        <ReactMarkdown key={`invalid-chart-${chartIndex}-${match.index}`} remarkPlugins={[remarkGfm]}>
          {`\`\`\`chart\n${rawChartData}\n\`\`\``}
        </ReactMarkdown>,
      );
    }
